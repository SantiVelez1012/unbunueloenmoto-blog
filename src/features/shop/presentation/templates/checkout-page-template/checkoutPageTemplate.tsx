"use client"

import React, { useState } from 'react'
import OrderSummary from '../../components/order-summary/orderSummary'
import CheckoutForm from '../../components/checkout-form/checkoutForm'
import { useCartStore } from '@/features/shop/infrastructure/state/cartStore'
import Link from 'next/link'
import { CheckoutFormInputs } from '../../models/checkoutModel';
import { CreateOrderUseCase } from '@/features/shop/domain/use-cases/create-order-use-case/createOrderUseCase'
import CheckoutConfirmationModal from '../../components/confirmation-modal/confirmationModal';
import { useRouter } from 'next/navigation';

function CheckoutPageTemplate() {
    const itemsCount = useCartStore((state) => state.items).length;
    const items = useCartStore(state => state.items);
    const { clearCart } = useCartStore(state => state);

    // Use the useRouter hook from next/navigation for client components
    const router = useRouter();

    const useCase = new CreateOrderUseCase();

    const [isLoading, setIsLoading] = useState(false);
    const [isSuccessOrderCreate, setIsSuccessOrderCreate] = useState<boolean | null>(null);

    function sendInfo(data: CheckoutFormInputs) {
        (document.getElementById('confirmation_modal') as HTMLDialogElement)!.showModal();
        setIsLoading(true);
        useCase.execute(data, items)
            .then(response => {
                if (response.ok) {
                    setIsSuccessOrderCreate(true);
                    clearCart();
                } else {
                    setIsSuccessOrderCreate(false);
                }
            })
            .finally(() => {
                setIsLoading(false);
            });
    }

    function handleCloseModal() {
        setIsSuccessOrderCreate(null);
        router.push('/shop');
    }

    return (

        <>
            <CheckoutConfirmationModal onClose={handleCloseModal} isLoading={isLoading} isSuccess={isSuccessOrderCreate} />

            {itemsCount === 0 && (
                <section className='min-h-[80dvh] flex flex-col items-center justify-center gap-0 mt-10'>
                    <h1 className="text-3xl font-bold mb-10 text-center">Tu carrito está vacío</h1>
                    <p className='text-lg font-medium mb-10 text-center'>Agrega productos a tu carrito para proceder con la compra.</p>
                    <Link href={'/shop'} className='btn btn-warning'> Volver al inicio </Link>
                </section>
            )}

            {itemsCount > 0 && (
                <section className='my-10'>
                    <h1 className="text-3xl font-bold mb-10 text-center">Finalización de compra</h1>


                    <span>
                        <p className='text-lg font-medium mb-10 text-center'>Recuerda que todos los envíos son contraentrega, asi que el pago lo haces cuando recibas tu pedido</p>
                    </span>

                    <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8">
                        <CheckoutForm submit={sendInfo} />
                        <OrderSummary />
                    </div>
                </section>
            )}
        </>


    )
}

export default CheckoutPageTemplate;